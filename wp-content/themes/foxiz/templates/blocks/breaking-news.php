...
if ( ! function_exists( 'foxiz_get_breaking_news' ) ) {
	 ...
	 * @return false|string
	 */
	function foxiz_get_breaking_news( $settings = array(), $_query = null ) {
	...
	function foxiz_loop_breaking_news( $settings, $_query ) {
		...
		if ( $_query->post_count > 1 ) : ?>
			<?php if ( ! empty( $settings['heading'] ) ) : ?>
                <span class="breaking-news-heading"><?php echo foxiz_get_svg( 'fire' ) . '<span>' . esc_html( $settings['heading'] ) . '</span>'; ?></span>
			<?php endif; ?>
