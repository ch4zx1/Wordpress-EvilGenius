...
if ( ! function_exists( 'foxiz_render_user_form_popup' ) ) {
	...
	function foxiz_render_user_form_popup() {
		...
		if ( is_user_logged_in() || ( empty( $settings['header_login_icon'] ) && empty( 'mobile_login' ) ) ) {
			return false;
		}
		if ( ! empty( $settings['header_login_redirect'] ) ) {
			$args['redirect'] = $settings['header_login_redirect'];
		}
		if ( ! empty( $settings['header_login_register'] ) ) {
			$args['register'] = $settings['header_login_register'];
		}
		if ( ! empty( $settings['header_login_forget'] ) ) {
			$args['forget_password'] = $settings['header_login_forget'];
		}
		?>
...
if ( ! function_exists( 'foxiz_get_adblock_popup' ) ) {
	...
	function foxiz_get_adblock_popup() {
		...
		$output .= '<div class="site-access-btn"><a class="is-btn" href="' . home_url( add_query_arg( array(), $wp->request ) ) . '">' . foxiz_html__( 'Okay, I\'ll Whitelist' ) . '</a>' . '</div>';
		$output .= '</div>';
		$output .= '</aside>';
